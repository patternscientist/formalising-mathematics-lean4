...
example : P → P := by
  -- The goal is of the form `X → Y`, so we can use `intro`
  intro h
  -- `h` is a proof of `P`, which is exactly our goal
  exact h
  done
...
example : (P → Q) → ((P → Q) → P) → Q := by
  -- the goal has the form `X -> Y -> Z`, so we can use `intro`
  intro hPQ hPQP
  -- now the goal is `⊢ Q`. `hPQ` says that `P` implies `Q` so it
  -- suffices to show `P`
  apply hPQ
  -- `hPQ` says `P` implies `Q`, and `hPQP` says that (`P` implies `Q`) implies `P`,
  -- hence `apply hPQP at hPQ` will change `hPQ` to a proof of `Q`, which is the goal
  exact hPQP hPQ
  done
