...
-- I (ddp) wrote these injective and surjective functions for use in this file
theorem injective_def (A B : Type) (f : A → B) : Injective f ↔ ∀ a1 a2 : A, f a1 = f a2 → a1 = a2 := by
  rfl


theorem surjective_def (A B : Type) (f : A → B) : Surjective f ↔ ∀ b : B, ∃ a : A, f a = b := by
  rfl
...
example : ¬∀ A B C : Type, ∀ (φ : A → B) (ψ : B → C), Injective (ψ ∘ φ) → Injective ψ := by
  ...
  have hgf : Injective (g ∘ f) := gf_injective
  have hg : Injective g := h hgf
  rw [injective_def Y Z g] at hg
  specialize hg Y.b Y.c
  apply Yb_ne_Yc
  apply hg
  exact gYb_eq_gYc
...
example : ¬∀ A B C : Type, ∀ (φ : A → B) (ψ : B → C), Surjective (ψ ∘ φ) → Surjective φ := by
  intro h
  specialize h X Y Z f g
  have hf := h gf_surjective
  rw [surjective_def X Y f] at hf
  specialize hf Y.c
  cases' hf with a ha
  apply Yb_ne_Yc
  exact ha
